...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BalanceCard(props) {
			...
			// 峰谷徽标在切换点精确翻牌：一次性定时器对齐下一个边界（分钟粒度 +
			// 1.5s 缓冲），翻牌后重新排程下一次；不引入额外轮询。
			const [, setPeakTick] = react.useState(0);
			react.useEffect(() => {
				let timer;
				let disposed = false;
				const schedule = () => {
					const now = Date.now();
					const waitMs = (deepSeekPeakInfo(now).minutesLeft * 60 - new Date(now).getUTCSeconds()) * 1000 + 1500;
					timer = setTimeout(() => {
						if (disposed) return;
						setPeakTick((n) => n + 1);
						schedule();

 ...
 *  - 收集全部非空白会话的 provider（经 connection.api.sessions.models，
 *    callUnary 的 result.value 契约）与其最近活跃时间（sessions.list
 *    快照的 updatedAt），按「该 provider 名下会话的最大 updatedAt」降序
 *    取前 3 个展示；不足 3 个不硬凑；
 *  - 增量缓存：仅新会话触发查询，稳态轮询零额外 RPC；会话活跃度变化
 *    直接刷新快照时间戳；
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BalanceCard(props) {
			...
			posRef.current = pos;
			// 会话 id → { provider, updatedAt } 增量缓存（provider 为 null 表示已查过但无模型）。
			const knownSessions = react.useRef(new Map());

			/** 最近使用的 provider（≤3 个，按名下会话最大 updatedAt 降序；增量缓存，稳态零 RPC）。 */
			const collectRecentProviders = react.useCallback(async () => {
				...
				const active = snapshot.ids.filter((id) => {
					const row = snapshot.byId && snapshot.byId[id];
					return !(row && row.blank === true);
				});
				// 清理已消失的会话。
				for (const id of [...knownSessions.current.keys()]) {
					if (!active.includes(id)) knownSessions.current.delete(id);
				}
				// 刷新已知会话的最近活跃时间（updatedAt 来自列表快照，零 RPC）。
				for (const id of active) {
					const fact = knownSessions.current.get(id);
					if (fact && fact.provider) {
						const row = snapshot.byId && snapshot.byId[id];
						if (row && typeof row.updatedAt === "number" && row.updatedAt > fact.updatedAt) {
							knownSessions.current.set(id, { ...fact, updatedAt: row.updatedAt });
						}
					}
				}
				// 仅查询新增会话（callUnary 契约：result.ok → result.value.current.provider）。
				const fresh = active.filter((id) => !knownSessions.current.has(id));
				if (fresh.length > 0) {
					const results = await Promise.all(fresh.map((sessionId) =>
						connection.api.sessions.models({ sessionId })
							.then((resp) => {
								const r = resp && resp.result;
								return r && r.ok && r.value && r.value.current ? r.value.current.provider : null;
							})
							.catch(() => null)
					));
					fresh.forEach((id, i) => {
						const row = snapshot.byId && snapshot.byId[id];
						knownSessions.current.set(id, {
							provider: results[i],
							updatedAt: row && typeof row.updatedAt === "number" ? row.updatedAt : 0,
						});
					});
				}
				// 每 provider 最近使用时间 = 名下会话最大 updatedAt；降序取前 MAX_RECENT_PROVIDERS。
				const byProvider = new Map();
				for (const fact of knownSessions.current.values()) {
					if (!fact || !fact.provider) continue;
					const prev = byProvider.get(fact.provider);
					if (prev === void 0 || fact.updatedAt > prev) byProvider.set(fact.provider, fact.updatedAt);
				}
			...
			// 会话列表变化：集合变化时清缓存全量重查一次；模型切换由轮询兜底（≤ refreshMs）。
			react.useEffect(() => {
				...
				return sessions.list.subscribe(() => {
					...
					const known = new Set(knownSessions.current.keys());
					if (ids.length !== known.size || ids.some((id) => !known.has(id))) {
						knownSessions.current.clear();
					}
					refresh();
				});
			}, [sessions, refresh]);
		...
		function apply(ctx, config) {
			...
			ctx.inject(["slots", "connection", "sessions"], (scope) => {
				scope.slots.inject("shell.overlay", () => scope.slots.register({
					name: "shell.overlay",
					id: "llm-balance",
					inject: () => ({ refreshMs, connection: scope.connection, sessions: scope.sessions }),
				}, BalanceCard));
			});
		}
